...
The day-to-day of *building* this - running the measurements, what the numbers
actually say, the checks that guard each claim, and the things that turned out
to be wrong - lives in **[NOTES.md](NOTES.md)**. It is a lab notebook, not a
manual: read it if you want to change the simulation or argue with a figure.
...
lands partly on the predator, and the panel printed *"a predator, +0.28"* in
green, which reads as though the creature enjoys them. A hunter can never
*make* a moment good (the only lesson predators hand out is negative, and they
give no reward at all), so **a good surprise never gets the predator's name**:
it falls through to whatever else was in the frame — the meal that actually
drove it, or just "a moment". No line saying "a predator" is ever green again.
The underlying learning was checked rather than assumed: over 5 seeds,
`V(predator)` and the flock's dread were both unmoved by the bigger food reward
— neither gap came close to its bar — so the leak was only ever in the wording.
